...
export function friendlyError(raw: string | null | undefined): string {
  ...
  // Strip internal function-name prefixes like "signIn: ...", "signUp: ..." etc.
  const stripped = raw.replace(/^[a-zA-Z]+:\s*/, '');

  // Strip "Sign in failed: ", "Sign up failed: " etc.
  const cleaned = stripped.replace(/^(sign in|sign up|reset password|update password|failed to \w+):\s*/i, '');

  // Check HTTP status code pattern
  ...
  // If the cleaned message is short and readable (no technical jargon), use it directly
